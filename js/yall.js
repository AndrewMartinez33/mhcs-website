...
var yall = function(userOptions) {
  ...
  let yallLoad = function(element) {
    ...
    if (element.tagName === "IMG") {
      ...
      if (parentElement.tagName === "PICTURE") {
        [].slice
          .call(parentElement.querySelectorAll("source"))
          .forEach(source => yallFlipDataAttrs(source));
      }
    ...
    if (element.tagName === "VIDEO") {
      [].slice
        .call(element.querySelectorAll("source"))
        .forEach(source => yallFlipDataAttrs(source));
      element.load();
    }
    ...
    if (active === false && lazyElements.length > 0) {
      ...
      setTimeout(() => {
        lazyElements.forEach(lazyElement => {
          if (
            lazyElement.getBoundingClientRect().top <=
              window.innerHeight + options.threshold &&
            lazyElement.getBoundingClientRect().bottom >= -options.threshold &&
        ...
        if (lazyElements.length === 0 && options.observeChanges === false) {
          env.eventsToBind.forEach(eventPair =>
  ...
  const selectorString = `img.${options.lazyClass},video.${
    options.lazyClass
  },iframe.${options.lazyClass},.${options.lazyBackgroundClass}`;
  ...
  if (env.intersectionObserverSupport === true) {
    var intersectionListener = new IntersectionObserver(
      (entries, observer) => {
  ...
  if (env.mutationObserverSupport === true && options.observeChanges === true) {
    const mutationListener = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        [].slice
          .call(document.querySelectorAll(selectorString))
          .forEach(newElement => {
    ...
    mutationListener.observe(
      document.querySelector(options.observeRootSelector),
